...
function parseAppSend(jsonData) //根据产品通信协议，解析从云端下发的协议数据
{
  ...
  if(jsonData.params.data.cmdId == 2)
  {
    if(jsonData.params.data.power == 1)
    {
      switchStatus = 1;
      reportStatus(device.mainTCPLink);
    }
    else
    {
      switchStatus = 0;
  ...
  if(jsonData.params.data.cmdId == 0)
  {
    reportStatus(device.mainTCPLink);    
  }      
}
...
function reportStatus(tcplink) //根据产品通信协议，上报设备当前状态
{
  ...
  if(switchStatus == 0)
  {
    sendParamsData("\"cmdId\":1,\"power\":0",tcplink);
  }
  else
  { 
    sendParamsData("\"cmdId\":1,\"power\":1",tcplink);
  }   
}
